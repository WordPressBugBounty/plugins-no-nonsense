...
function r34nono_admin_bar_logout_link() {
	add_action('admin_bar_menu', 'r34nono_admin_bar_logout_link_admin_bar_menu_callback', PHP_INT_MAX - 100);
...
function r34nono_auto_core_update_send_email_only_on_error($send, $type, $core_update, $result) {
	$options = get_option('r34nono_auto_core_update_send_email_only_on_error_options');
	if (isset($options['send_critical']) && !empty($options['send_critical'])) {
		return (empty($type) || $type != 'success');
	}
	return (!empty($type) && $type == 'fail');
}
...
function r34nono_disable_author_archives() {
	// Return 404 error for author archives
	add_action('template_redirect', 'r34nono_disable_author_archives_template_redirect_callback', 1, 0);
	// Remove "View" links from Users admin page
	add_filter('user_row_actions', 'r34nono_disable_author_archives_user_row_actions_callback', PHP_INT_MAX - 1, 1);
	// Remove author archive pages from default sitemap
	add_filter('wp_sitemaps_add_provider', 'r34nono_disable_author_archives_wp_sitemaps_add_provider_callback', PHP_INT_MAX - 1, 2);
	// Remove author links in content
	add_filter('author_link', '__return_false', PHP_INT_MAX - 1);
	add_filter('the_author_posts_link', 'get_the_author', PHP_INT_MAX - 1);
}

function r34nono_disable_author_archives_template_redirect_callback() {
	if (is_author()) {
		global $wp_query;
		$wp_query->set_404();
		status_header(404);
...
function r34nono_disable_author_archives_user_row_actions_callback($actions) {
	if (isset($actions['view'])) { unset($actions['view']); }
	return $actions;
}

function r34nono_disable_author_archives_wp_sitemaps_add_provider_callback($provider, $name) {
	if ($name == 'users') { $provider = false; }
	return $provider;
}
...
function r34nono_disable_site_search() {
	...
	// @todo Troubleshoot JavaScript error
...
function r34nono_hide_admin_bar_for_logged_in_non_editors() {
	if (!wp_doing_ajax() && is_user_logged_in()) {
		$options = get_option('r34nono_hide_admin_bar_for_logged_in_non_editors_options');
		if (!current_user_can('edit_posts') || !empty($options['hide_for_all'])) {
...
function r34nono_plugins_jetpack_restrict_admin_access() {
	if (!current_user_can('edit_others_posts')) {
		remove_submenu_page('jetpack', 'akismet-key-config');
		remove_submenu_page('jetpack', 'jetpack_about');
		...
		remove_submenu_page('jetpack', 'jetpack#/settings');
		remove_submenu_page('jetpack', 'my-jetpack');
		remove_submenu_page('jetpack', 'stats');
		remove_menu_page('feedback');
		remove_menu_page('jetpack');
		remove_menu_page('jetpack_admin_page');
		remove_menu_page('jetpack_network_admin_page');
		remove_menu_page('jetpack-forms');
	}
}
...
function r34nono_remove_howdy($wp_admin_bar) {
	if ($my_account = $wp_admin_bar->get_node('my-account')) {
		$wp_admin_bar->add_node(array(
			'id' => 'my-account',
...
function r34nono_xmlrpc_disabled() {
	...
	/**
	 * Note: As of version 3.3.3 the logic to silently kill any XML-RPC request runs
	 * directly in no-nonsense.php as the plugin is loading.
	 */
}
